{if $addons.ee_change_winrace.ee_change_winrace_active == "Y"}
			...
			<div id="block_company_{$company_data.company_id}" class="clearfix">
				...
				<div class="ty-company-detail__info">
					...
					{capture name = "profile_fields_{$obj_id}"}
						...
						{/foreach}
					{/capture}
					{if $smarty.capture["profile_fields_{$obj_id}"]|trim && 1 == 0}
					...
					{if
						...
						<h2 class="ty-mainbox-title">{$company_data.company}</h2>
	...
	{/hook}
	<!-- /responsive/templates/addons/ee_change_winrace/hooks/companies/view.override.tpl -->
{else}
	where is it ???!!!
	<!-- /responsive/templates/addons/ee_change_winrace/hooks/companies/view.override.tpl -->
{/if}